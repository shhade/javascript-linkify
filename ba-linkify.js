...
window.linkify = (function(){
    var
     ...
     //SCHEME = "((?:https?|ftps?://|www\d{0,3}[.]|[a-z0-9.\-]+[.]+" + TLD + "/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))",//"[a-z\\d.-]+://",
     SCHEME = "(http|https|ftp|ftps|irc|file)+://",//"[a-z\\d.-]+://",
     //SCHEME2 = "[a-z\\d.-]+://",
     ...
     QUERY_FRAG = "(?:\\?[^#<>\\s]*)?(?:#[^<>\\s]*)?",
     PORT = "(?:\:[0-9]+)?",
     URI1 = SCHEME + HOSTNAME + TLD,//+ "[^<>\\s]+",
     URI2 = "\\b" + HOST_OR_IP + PORT + PATH + "(?!\\w))?" + QUERY_FRAG + "(?!\\w)",
     //URI2 = "\\b" + HOST_OR_IP + PORT + PATH + QUERY_FRAG + "(?!\\w)",
     URI3 = SCHEME + "[^<>\\s]+",
     //URI4 = "\\b" + SCHEME2 + "[^<>\\s]+",
     MAILTO = "mailto:",
     EMAIL = "(?:" + MAILTO + ")?[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@" + HOST_OR_IP + QUERY_FRAG + "(?!\\w)",

     URI_RE = new RegExp("(?:" + EMAIL + "|" + URI2 + "|" + URI3 + "|" + URI1 + ")", "ig"),
